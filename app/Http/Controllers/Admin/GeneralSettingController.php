...
class GeneralSettingController extends Controller
{
    ...
    {
        $request->validate([
            'logo'    => ['nullable', 'image', 'mimes:jpg,jpeg,png', new FileTypeValidate(['jpg', 'jpeg', 'png'])],
            'favicon' => ['nullable', 'image', 'mimes:png', new FileTypeValidate(['png']), 'max:2048'],
        ], [
            'favicon.image' => 'The favicon must be an image file.',
            'favicon.mimes' => 'The favicon must be a PNG file.',
        ...
        if ($request->hasFile('favicon')) {
            try {
                // التحقق من أن الملف صورة PNG صالحة
                $favicon = $request->file('favicon');
                $mimeType = $favicon->getMimeType();
                
                if (!in_array($mimeType, ['image/png', 'image/x-png'])) {
                    $notify[] = ['error', 'Favicon must be a valid PNG image file. Detected type: ' . $mimeType];
                    return back()->withNotify($notify);
                }
                
                // استخدام طريقة مباشرة للرفع بدون Intervention Image للـ favicon
                $oldFavicon = $path . '/favicon.png';
                if (file_exists($oldFavicon)) {
                    @unlink($oldFavicon);
                }
                
                // نسخ الملف مباشرة بدون معالجة
                $favicon->move($path, 'favicon.png');
