...
function getChargeText($chargeLimit) {
    $text = __('Charge').' ';
    
    if ($chargeLimit->fixed_charge) {
        $text .= showAmount($chargeLimit->fixed_charge);
        if ($chargeLimit->percent_charge || $chargeLimit->vat) {
            $text .= ' + ';
        }
    }
    
    if ($chargeLimit->percent_charge) {
        $text .= showAmount($chargeLimit->percent_charge, currencyFormat: false) . '%';
        if ($chargeLimit->vat) {
            $text .= ' & ';
        }
    }
    ...
    if ($chargeLimit->vat) {
        $text .= showAmount($chargeLimit->vat, currencyFormat: false) . '% ' . __('VAT');
    }
